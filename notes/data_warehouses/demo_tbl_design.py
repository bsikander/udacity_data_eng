...
gzip region 'us-west-2';
        """.format(
            table, table, DWH_ROLE_ARN
        )

        print(
            "======= LOADING TABLE: ** {} ** IN SCHEMA ==> {} =======".format(
                table, schema
            )
        )
        print(SQL_COPY)
    ...
    return pd.DataFrame({"table": tables, "loadtime_" + schema: loadTimes}).set_index(
        "table"
    )
...
def compareQueryTimes(schema):
    queryTimes = []
    for _, query in enumerate([oneDim_SQL, twoDim_SQL, drill_SQL, oneDimSameDist_SQL]):
        t0 = time()
